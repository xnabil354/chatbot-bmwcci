...
async function start() {
  ...
  zhn.ev.on("contacts.update", (update) => {
  ...
  // bind extra client
  await Client({ zhn, store });

  // login use pairing code
  // source code https://github.com/WhiskeySockets/Baileys/blob/master/Example/example.ts#L61
  if (pairingCode && !zhn.authState.creds.registered) {
    ...
    setTimeout(async () => {
      let code = await zhn.requestPairingCode(phoneNumber);
  ...
  if (useMobile && !zhn.authState.creds.registered) {
    const { registration } = zhn.authState.creds || { registration: {} };
    ...
    async function enterCode() {
      try {
        const code = await question(
          chalk.bgBlack(chalk.greenBright(`Please Enter Your OTP Code : `))
        );
        const response = await zhn.register(
          code
            .replace(/[^0-9]/g, "")
            .trim()
            .toLowerCase()
        ...
        console.log(
          chalk.bgBlack(
            chalk.greenBright("Successfully registered your phone number.")
          )
        );
    ...
    async function enterCaptcha() {
      ...
      await open(pathFile);
      const code = await question(
        chalk.bgBlack(chalk.greenBright(`Please Enter Your Captcha Code : `))
      );
    ...
    async function askOTP() {
      ...
      try {
        await zhn.requestRegistrationCode(registration);
        await enterCode();
      } catch (e) {
        console.error(
  ...
  zhn.ev.on("connection.update", async (update) => {
    ...
    if (connection === "open") {
      zhn.sendMessage(config.options.owner[0] + "@s.whatsapp.net", {
        text: `${zhn?.user?.name || "zhn"} has Connected...`,
      });
    }
  });

  // write session
  zhn.ev.on("creds.update", saveCreds);

  // messages
  zhn.ev.on("messages.upsert", async (message) => {
    if (!message.messages) return;
    const m = await Serialize(zhn, message.messages[0]);
    await (
      await import(`./event/message.js?v=${Date.now()}`)
    ).default(zhn, m, message);
  });

  // group participants update
  zhn.ev.on("group-participants.update", async (message) => {
    await (
  ...
  // group update
  zhn.ev.on("groups.update", async (update) => {
    await (
  ...
  zhn.ev.on("call", async (json) => {
    if (config.options.antiCall) {
      for (const id of json) {
        if (id.status === "offer") {
          let msg = await zhn.sendMessage(id.from, {
          ...
          zhn.sendContact(id.from, config.options.owner, msg);
          await zhn.rejectCall(id.id, id.from);
        }
      }
    }
  });
