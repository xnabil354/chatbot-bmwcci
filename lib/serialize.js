...
export function Client({ zhn, store }) {
   ...
   const client = Object.defineProperties(zhn, {
      getContentType: {
         value(content) {
            if (content) {
      ...
      getName: {
         value(jid) {
            let id = zhn.decodeJid(jid)
            let v
            if (id?.endsWith("@g.us")) return new Promise(async (resolve) => {
               v = zhn.contacts[id] || zhn.messages["status@broadcast"]?.array?.find(a => a?.key?.participant === id)
               if (!(v.name || v.subject)) v = zhn.groupMetadata[id] || {}
               resolve(v?.name || v?.subject || v?.pushName || (parsePhoneNumber('+' + id.replace("@g.us", "")).format("INTERNATIONAL")))
            })
            else v = id === "0@s.whatsapp.net" ? {
               id,
               name: "WhatsApp"
            } : id === zhn.decodeJid(zhn?.user?.id) ?
               zhn.user : (zhn.contacts[id] || {})
      ...
      sendContact: {
         async value(jid, number, quoted, options = {}) {
            ...
            for (let v of number) {
               list.push({
                  displayName: await zhn.getName(v),
                  vcard: `BEGIN:VCARD\nVERSION:3.0\nN:${await zhn.getName(v + "@s.whatsapp.net")}\nFN:${await zhn.getName(v + "@s.whatsapp.net")}\nitem1.TEL;waid=${v}:${v}\nitem1.X-ABLabel:Ponsel\nitem2.EMAIL;type=INTERNET:${config.Exif.packEmail}\nitem2.X-ABLabel:Email\nitem3.URL:${config.Exif.packWebsite}\nitem3.X-ABLabel:Instagram\nitem4.ADR:;;Indonesia;;;;\nitem4.X-ABLabel:Region\nEND:VCARD`
               })
            }
            return zhn.sendMessage(jid, {
               contacts: {
                  displayName: `${list.length} Contact`,
                  contacts: list
               },
               mentions: quoted?.participant ? [zhn.decodeJid(quoted?.participant)] : [zhn.decodeJid(zhn?.user?.id)],
               ...options
            }, { quoted, ...options })
         },
         enumerable: true
      ...
      sendMedia: {
         async value(jid, url, quoted = "", options = {}) {
            ...
            if (size > 45000000) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${zhn.user?.name} (${new Date()}).${ext}`, ...options }
            else if (options.asDocument) data = { document: buffer, mimetype: mime, fileName: options?.fileName ? options.fileName : `${zhn.user?.name} (${new Date()}).${ext}`, ...options }
            ...
            let msg = await zhn.sendMessage(jid, data, { quoted, ...options })
            return msg
         },
         enumerable: true
      },
      ...
      cMod: {
         value(jid, copy, text = '', sender = zhn.user.id, options = {}) {
            let mtype = zhn.getContentType(copy.message)
            ...
            copy.key.fromMe = areJidsSameUser(sender, zhn.user.id)
      ...
      sendPoll: {
         async value(chatId, name, values, options = {}) {
            ...
            return await zhn.sendMessage(chatId, {
               poll: {
                  name,
                  values,
                  selectableCount
      ...
      sendGroupV4Invite: {
         async value(jid, groupJid, inviteCode, inviteExpiration, groupName, jpegThumbnail, caption = "Invitation to join my WhatsApp Group", options = {}) {
            const media = await prepareWAMessageMedia({ image: (await Function.getFile(jpegThumbnail)).data }, { upload: zhn.waUploadToServer })
            ...
            const m = generateWAMessageFromContent(jid, message, { userJid: zhn.user?.id })
            await zhn.relayMessage(jid, m.message, { messageId: m.key.id })
...
export async function Serialize(zhn, msg) {
   const m = {}
   const botNumber = zhn.decodeJid(zhn.user?.id)
   ...
   if (msg.key) {
      m.key = msg.key
      m.from = zhn.decodeJid(m.key.remoteJid)
      ...
      m.sender = zhn.decodeJid(m.fromMe ? zhn.user.id : m.isGroup ? m.participant : m.from)
   }

   m.pushName = msg.pushName
   m.isOwner = m.sender && [...config.options.owner, botNumber.split`@`[0]].includes(m.sender.replace(/\D+/g, ""))
   if (m.isGroup) {
      m.metadata = await zhn.groupMetadata(m.from)
   ...
   if (m.message) {
      m.type = zhn.getContentType(m.message) || Object.keys(m.message)[0]
      ...
      m.reply = async (text, options = {}) => {
         ...
         if ((Buffer.isBuffer(text) || /^data:.?\/.*?;base64,/i.test(text) || /^https?:\/\//.test(text) || fs.existsSync(text))) {
            ...
            if (!options.mimetype && (/utf-8|json/i.test(data.mime) || data.ext == ".bin" || !data.ext)) {
               ...
               return zhn.sendMessage(chatId, { text, mentions: [m.sender, ...zhn.parseMention(text)], ...options }, { quoted, ephemeralExpiration: m.expiration, ...options })
            } else {
               return zhn.sendMedia(m.from, data.data, quoted, { ephemeralExpiration: m.expiration, ...options })
            }
         } else {
            if (!!config.msg[text]) text = config.msg[text]
            return zhn.sendMessage(chatId, { text, mentions: [m.sender, ...zhn.parseMention(text)], ...options, }, { quoted, ephemeralExpiration: m.expiration, ...options });
      ...
      m.download = (filepath) => {
         if (filepath) return zhn.downloadMediaMessage(m, filepath)
         else return zhn.downloadMediaMessage(m)
      }
   }
   ...
   if (m.msg?.contextInfo?.quotedMessage) {
      ...
      if (m.quoted.message) {
         m.quoted.type = zhn.getContentType(m.quoted.message) || Object.keys(m.quoted.message)[0]
         m.quoted.msg = extractMessageContent(m.quoted.message[m.quoted.type]) || m.quoted.message[m.quoted.type]
         m.quoted.key = {
            remoteJid: m.msg?.contextInfo?.remoteJid || m.from,
            participant: m.msg?.contextInfo?.remoteJid?.endsWith("g.us") ? zhn.decodeJid(m.msg?.contextInfo?.participant) : false,
            fromMe: areJidsSameUser(zhn.decodeJid(m.msg?.contextInfo?.participant), zhn.decodeJid(zhn?.user?.id)),
         ...
         m.quoted.sender = zhn.decodeJid(m.msg?.contextInfo?.participant)

         m.quoted.isOwner = m.quoted.sender && [...config.options.owner, botNumber.split`@`[0]].includes(m.quoted.sender.replace(/\D+/g, ""))
         if (m.quoted.isGroup) {
            m.quoted.metadata = await zhn.groupMetadata(m.quoted.from)
         ...
         m.quoted.download = (filepath) => {
            if (filepath) return zhn.downloadMediaMessage(m.quoted, filepath)
            else return zhn.downloadMediaMessage(m.quoted)
         }
      }
   }
